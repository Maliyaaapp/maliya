...
import { Plus, CreditCard, Printer, Download, Edit, MessageSquare, Calendar, AlertCircle, Trash, Pause, Play } from 'lucide-react';
...
const SubscriptionsList = () => {
  ...
  const generateSampleSubscriptions = (schools: School[]): Subscription[] => {
    return schools.map((school) => {
      ...
      // Status
      let status: 'active' | 'expired' | 'pending' | 'paused' = 'active';
      const now = new Date();
      if (endDate < now) {
        status = 'expired';
      } else if (!paid) {
