...
const Settings = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Use either actual schoolId or temporary one
    const effectiveSchoolId = user?.schoolId || tempSchoolId;
    console.log('Using schoolId:', effectiveSchoolId);
    
    try {
      if (effectiveSchoolId) {
        console.log('Updating school settings with ID:', effectiveSchoolId);
        const response = await api.updateSchoolSettings(effectiveSchoolId, settings);
        ...
        if (response.success) {
          ...
          if (user && updateUserInfo) {
            console.log('Updating user info with new school details');
            try {
              await updateUserInfo({
                ...user,
                schoolId: effectiveSchoolId, // Add the school ID to user if it wasn't there
              ...
              if (tempSchoolId) {
                alert('تم إنشاء مدرسة جديدة وحفظ الإعدادات بنجاح. سيتم تحديث الصفحة.');
        ...
        console.error('No schoolId available for saving settings');
        alert('لا يمكن حفظ الإعدادات: المعرف غير متوفر');
      }
    } catch (error) {
