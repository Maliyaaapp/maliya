...
const InstallmentForm = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const installmentAmount = formData.amount / formData.installmentCount;
      ...
      for (const installment of installments) {
        await dataStore.saveInstallment(installment);
      }
      ...
      setErrors({
        submit: 'حدث خطأ أثناء حفظ القسط. يرجى المحاولة مرة أخرى.'
      });
    } finally {
      setIsSaving(false);
    }
                ...
                <div>
                  ...
                  {formData.installmentCount > 1 && (
                    <p className="text-sm text-gray-500 mt-1">
                      قيمة القسط الواحد: {(formData.amount / formData.installmentCount).toFixed(2)} {CURRENCY}
                    </p>
                  )}
                </div>
              </div>
