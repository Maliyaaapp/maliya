...
const StudentForm = () => {
  ...
  useEffect(() => {
    if (isEditMode && id) {
      ...
      if (student) {
        ...
        const studentFees = dataStore.getFees(student.schoolId, student.id);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (formData.transportation !== 'none') {
        ...
        const existingFees = dataStore.getFees(user?.schoolId || '', savedStudent.id);
        ...
        if (!existingTransportationFee || isEditMode) {
          const feeData = {
            ...
            balance: transportationAmount - (existingTransportationFee?.paid || 0),
      ...
      // If tuition fee is selected, create or update tuition fee
      if (showTuitionFeeSection && formData.tuitionFee) {
                ...
                <div className="p-4 bg-gray-50 rounded-md mt-2 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-gray-700 mb-2" htmlFor="tuitionFee">
                        الرسوم الدراسية <span className="text-red-500">*</span>
                      </label>
                      <div className="relative">
                        <input
                          id="tuitionFee"
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          value={formData.tuitionDiscount || 0}
                          onChange={handleChange}
                          min="0"
                        />
                        <div className="absolute inset-y-0 left-0 flex items-center bg-gray-100 border-l border-gray-300 px-3 rounded-l-md">
                  ...
                  <div className="p-3 bg-white rounded border border-gray-200 mt-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-primary">
                        {((formData.tuitionFee || 0) - (formData.tuitionDiscount || 0)).toLocaleString()} {CURRENCY}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 mt-2">
                      سيتم إنشاء رسوم دراسية تلقائياً للطالب بعد الحفظ
                    </p>
                  </div>
                </div>
              )}
