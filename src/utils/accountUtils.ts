...
export interface SyncStatusResponse {
  ...
  data?: {
    ...
    totalAppwriteAccounts: number;
    localOnlyAccounts: AccountData[];
    appwriteOnlyAccounts: AccountData[];
    isSynced: boolean;
  };
}
...
export interface FixResult {
  ...
  error?: string;
  data?: {
    fixedLocalAccounts: string[];
    fixedAppwriteAccounts: string[];
    errors: Array<{
      type: string;
      id: string;
      error: string;
...
export const checkAccountSyncStatus = async (): Promise<SyncStatusResponse> => {
  try {
    ...
    const localAccounts: AccountData[] = JSON.parse(localStorage.getItem('accounts') || '[]');
    ...
    const appwriteAccounts: AccountData[] = appwriteResponse.documents.map(doc => ({
      id: doc.$id,
      email: doc.email,
...
export const fixAccountSyncIssues = async (): Promise<FixResult> => {
  try {
    const syncStatus = await checkAccountSyncStatus();
    
    if (!syncStatus.success) {
      return syncStatus as FixResult;
    }
    
    const { localOnlyAccounts, appwriteOnlyAccounts } = syncStatus.data!;
    const fixedLocalAccounts: string[] = [];
    const fixedAppwriteAccounts: string[] = [];
